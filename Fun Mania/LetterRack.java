...
public class LetterRack extends JPanel
{
    ...
    public LetterRack()
    {
        this("list", "./images/", ".png");
    }
    
    /**
     * Creates a new LetterRack given the list to be guessed, letter image
     * directory, and letter image type
     * @param inList The list to be guessed.
     * @param imageDirectory The directory of the letter images.
     * @param imageType The type of the letter images.
     */
    public LetterRack(String inList, String imageDirectory, 
            String imageType)
    {
        RACK_COLS = 8;
        RACK_ROWS = 2;
    ...
    /**
     * Builds a letter rack from a blend of the list and random letters.
     */
    private void buildRack()
    {
        StringBuilder listBuilder = 
                new StringBuilder(list.toLowerCase());
        ArrayList<Character> tiles = new ArrayList<>(); // cannot use char
        Random rand = new Random();
        int i = 0, j = 0;
        
        // add the list letters to the rack
        while (listBuilder.length() > 0)
        {
            // want to make sure that no letters are repeated in tile rack
            if (!tiles.contains(listBuilder.charAt(0)))
            {
                tiles.add(listBuilder.charAt(0));
                i++;
            }
            listBuilder.deleteCharAt(0);
        }
